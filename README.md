<div align="center">

```
██████╗  █████╗ ██╗      █████╗ ██████╗ ██╗███████╗██╗  ██╗██╗████████╗██╗  ██╗
██╔══██╗██╔══██╗██║     ██╔══██╗██╔══██╗██║██╔════╝██║  ██║██║╚══██╔══╝██║  ██║
██████╔╝███████║██║     ███████║██████╔╝██║███████╗███████║██║   ██║   ███████║
██╔══██╗██╔══██║██║     ██╔══██║██╔══██╗██║╚════██║██╔══██║██║   ██║   ██╔══██║
██████╔╝██║  ██║███████╗██║  ██║██║  ██║██║███████║██║  ██║██║   ██║   ██║  ██║
╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝╚══════╝╚═╝  ╚═╝╚═╝   ╚═╝   ╚═╝  ╚═╝
```

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&duration=3000&pause=800&color=00FF41&center=true&vCenter=true&width=620&lines=ML+%2B+Cybersecurity+Engineer+%F0%9F%A4%96%F0%9F%94%90;Built+a+SIEM+%2F+UEBA+Pipeline+from+scratch;CatBoost+%7C+IsolationForest+%7C+SHAP+%7C+OWASP;CTF+Player+%7C+Ethical+Hacker+in+Training;Catch+me+if+you+can...+%F0%9F%98%88" alt="Typing SVG" />

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/balarishith-balli-51443b261/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:balligowri70@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/balarishithballi)
[![Portfolio](https://img.shields.io/badge/Portfolio-00FF41?style=for-the-badge&logo=firefox&logoColor=black)](https://github.com/balarishithballi/portfolio)

<img src="https://komarev.com/ghpvc/?username=balarishithballi&style=flat-square&color=00ff41&label=PROFILE+VIEWS" />

</div>

---

<img align="right" alt="Hacking GIF" width="320" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif"/>

## `> cat whoami.txt`

```bash
╔═══════════════════════════════════════════════╗
║  NAME     │ Balarishith Balli                 ║
║  ROLE     │ ML + Cybersecurity Engineer       ║
║  LOCATION │ India  🇮🇳                        ║
║  EMAIL    │ balligowri70@gmail.com            ║
║  STATUS   │ Building. Breaking. Defending. 🚀 ║
╠═══════════════════════════════════════════════╣
║  "The quieter you become,                     ║
║   the more you are able to hear." — Kali      ║
╚═══════════════════════════════════════════════╝
```

**FUN FACT ⚡** The first virus `Creeper` (1971) just said _"I'm the creeper, catch me if you can!"_
`Reaper` was written to delete it — making it the world's first antivirus. 👾

<br clear="right"/>

---

## `> ls /skills --all`

<div align="center">

### 🤖 ML & AI
![CatBoost](https://img.shields.io/badge/CatBoost-FFCC00?style=for-the-badge&logo=yandex&logoColor=black)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?style=for-the-badge&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![SHAP](https://img.shields.io/badge/SHAP-Explainability-00ff41?style=for-the-badge)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white)

### 🔐 Security Arsenal
![Kali Linux](https://img.shields.io/badge/Kali_Linux-557C94?style=for-the-badge&logo=kali-linux&logoColor=white)
![Metasploit](https://img.shields.io/badge/Metasploit-2596CD?style=for-the-badge&logoColor=white)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge&logo=wireshark&logoColor=white)
![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6633?style=for-the-badge&logoColor=white)
![OWASP](https://img.shields.io/badge/OWASP-000000?style=for-the-badge&logo=owasp&logoColor=white)

### 💻 Development
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

</div>

---

## `> sudo cat /projects/flagship.md`

<div align="center">

### 🔰 VANGUARD — AI-Powered SIEM / UEBA Security Pipeline

</div>

> **The centrepiece of the portfolio.** A production-grade, end-to-end security analytics system fusing supervised ML, unsupervised anomaly detection, and explainable AI into a real SOC pipeline — built entirely from scratch.

<div align="center">

[![VANGUARD](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=VANGAURD_&theme=github_dark&bg_color=0d1117&border_color=00ff41&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/VANGAURD_)

</div>

```
RAW LOGS ──► LOG PARSER ──► DATA CLEANER ──► FEATURE ENGINEERING
                                                      │
                              ┌───────────────────────┤
                              ▼                       ▼
                       CatBoost Classifier     IsolationForest
                       (Supervised ML)         (UEBA Anomaly)
                              │                       │
                              └───────────┬───────────┘
                                          ▼
                                   RISK SCORING ENGINE
                                   (Fused 0–100 Score)
                                          │
                              ┌───────────┴───────────┐
                              ▼                       ▼
                        SHAP Explainer         SOC Dashboard
                        (Why this alert?)      (Streamlit UI)
```

<details>
<summary><b>🤖 ML Stack</b></summary>
<br/>

| Component | Technology | Purpose |
|:---|:---|:---|
| Supervised Classifier | `CatBoost` + `XGBoost` + Logistic Regression | Threat probability per event |
| Anomaly Detector | `IsolationForest` | UEBA behavioral anomaly scoring |
| Text Features | `HashingVectorizer` (1–3 gram, 32K features) | Sparse NLP on raw log text |
| Explainability | `SHAP` | Per-event feature attribution for SOC analysts |
| Scaling | `StandardScaler` + `RobustScaler` | Feature normalization |
| Class Balancing | Custom upsampling | Handle imbalanced threat datasets |
| Risk Fusion | Weighted sigmoid formula | Combine ML + UEBA + TI signals → 0–100 score |

</details>

<details>
<summary><b>📊 Performance Metrics (Live Test Run)</b></summary>
<br/>

```
╔══════════════════════════════════════════════╗
║  METRIC              VALUE                  ║
╠══════════════════════════════════════════════╣
║  Total Events        154,332                ║
║  True Positives      20,224   ✅            ║
║  True Negatives      134,102  ✅            ║
║  False Positives     6        🟡            ║
║  False Negatives     0        🏆 ZERO       ║
║  FP Rate             4.47e-05 (near-zero)   ║
║  UEBA Anomalies      2,636                  ║
╚══════════════════════════════════════════════╝
```
> Zero false negatives. Near-zero false positive rate. Every real threat caught.
</details>

<details>
<summary><b>🛡️ Attack Types Detected (14 categories)</b></summary>
<br/>

```python
VANGUARD_DETECTS = [
    "SQL Injection",    "XSS",              "Brute Force",
    "RCE",              "Path Traversal",   "Command Injection",
    "Malware",          "Ransomware",        "Port Scanning",
    "DoS / DDoS",       "Privilege Escalation", "Credential Dumping",
    "Lateral Movement", "Data Exfiltration"
]
```
</details>

---

## `> cat /projects/featured.json`

<details>
<summary><b>🧪 OWASP Immersive Vulnerable Lab 2025 (IVL)</b> &nbsp; ⭐ 1</summary>
<br/>

> A hands-on, self-hosted vulnerable web lab covering real OWASP Top 10 vulnerabilities. Built with PHP — designed to be exploited, not protected.

**Modules:** `SQLi` · `XSS` · `Command Injection` · `File Upload` · `Path Traversal`

[![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=OWASP-Immersive-Vulnerable-Lab-2025-OWASP-IVL-2025-&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/OWASP-Immersive-Vulnerable-Lab-2025-OWASP-IVL-2025-)

</details>

<details>
<summary><b>🔡 Custom Payload Encoder & Obfuscation Framework</b> &nbsp; ⭐ 1</summary>
<br/>

> A Streamlit-based research tool studying how payloads survive encoding transformations. Explores Base64, XOR, ROT13, Shannon entropy analysis, and pattern detection — essential for understanding signature bypass.

**Stack:** `Python` · `Streamlit` · `Shannon Entropy` · `Jaccard Similarity` · `Regex Pattern Matching`

[![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=Custom-Payload-Encoder-Obfuscation-Framework&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/Custom-Payload-Encoder-Obfuscation-Framework)

</details>

<details>
<summary><b>🔐 INFILTR8 — Python Security Toolkit</b> &nbsp; ⭐ 2 &nbsp; 🍴 1</summary>
<br/>

> Python-based security toolkit for network reconnaissance and vulnerability analysis. Because understanding offense is the foundation of defense.

[![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=INFILTR8&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/INFILTR8)

</details>

<details>
<summary><b>📡 WiFi Analyzer — Network Intelligence</b></summary>
<br/>

> Real-time wireless network analysis tool. Know your network topology, own your security posture.

[![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=wifi-analyzer&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/wifi-analyzer)

</details>

<details>
<summary><b>🚩 CTF Writeups & OverTheWire Series</b></summary>
<br/>

> Capture The Flag challenge writeups and full OverTheWire wargame series solutions. The best way to learn security is to break things (legally).

[![CTFs](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=ctfs&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/ctfs)
[![OTW](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=overthewire-series-&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/overthewire-series-)

</details>

<details>
<summary><b>🎮 Super Mario Lite — Python Game</b></summary>
<br/>

> Proof that security folks know how to have fun — a full Mario-style game in Python from scratch.

[![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=balarishithballi&repo=SUPERMARIO-LITE-&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41)](https://github.com/balarishithballi/SUPERMARIO-LITE-)

</details>

---

## `> htop --github`

<div align="center">

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=balarishithballi&show_icons=true&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&icon_color=00ff41&text_color=c9d1d9&hide_border=false&count_private=true" />
<img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=balarishithballi&theme=github-dark-blue&background=0d1117&border=21262d&ring=00ff41&fire=ff6b35&currStreakLabel=00ff41&sideLabels=8b949e" />

<img width="50%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=balarishithballi&layout=donut&theme=github_dark&bg_color=0d1117&border_color=21262d&title_color=00ff41&text_color=c9d1d9&hide_border=false" />

</div>

---

## `> tail -f /var/log/learning.log`

```
[ACTIVE]   ████████████████░░░░  SIEM / UEBA Pipeline Development
[ACTIVE]   ████████████████░░░░  OWASP Top 10 & Vulnerable Labs
[ACTIVE]   █████████████░░░░░░░  ML for Security (CatBoost, SHAP, IsolationForest)
[ACTIVE]   ████████████░░░░░░░░  CTF Challenges & OverTheWire Wargames
[ACTIVE]   ██████████░░░░░░░░░░  Payload Encoding & Obfuscation Research
[QUEUE]    ░░░░░░░░░░░░░░░░░░░░  CEH / eJPT Certification
[QUEUE]    ░░░░░░░░░░░░░░░░░░░░  Bug Bounty Hunting
```

---

## `> ping -c 1 balarishithballi`

```
PING balarishithballi.dev — I read every message.
64 bytes: response_time=<fast>   ttl=always-learning
```

<div align="center">

[![LinkedIn](https://img.shields.io/badge/Let's_Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/balarishith-balli-51443b261/)
&nbsp;
[![Email](https://img.shields.io/badge/Drop_a_Mail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:balligowri70@gmail.com)

<br/>

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=00ff41&height=80&section=footer" width="100%"/>

<sub>
  <code>// crafted with 💚 and a healthy dose of paranoia</code><br/>
  <code>// "In a world full of Creepers, be the Reaper." 🛡️</code>
</sub>

</div>
